...
use precompile_utils::{
    generate_function_selector, keccak256, revert, succeed, Address, Bytes, EvmDataReader,
...
impl<Runtime, Metadata: Erc20Metadata, Instance> Default
    ...
    <Runtime as pallet_timestamp::Config>::Moment: Into<U256>,
{
    fn execute(handle: &mut impl PrecompileHandle) -> EvmResult<PrecompileOutput> {
        let selector = handle.read_selector().unwrap_or(Action::Deposit);

        handle
            .check_function_modifier(match selector {
