...
impl<T: Config> Pallet<T> {
    /// Sender stakes KSMs to the validator and receives xKSMs in exchange
    ///
    /// Ensured atomic.
    #[transactional]
    pub fn stake_internal(who: &T::AccountId, amount: Balance) -> DispatchResult {
        T::Currency::transfer(CurrencyId::KSM, who, &Self::account_id(), amount)?;
        T::Currency::transfer(CurrencyId::xKSM, &Self::account_id(), who, amount)?;
        Ok(())
    }
    /// Sender redeems xKSMs in exchange for pending balance(Dot)
    ///
    /// Ensured atomic.
    #[transactional]
    pub fn unstake_internal(nominator: &T::AccountId, amount: Balance) -> DispatchResult {
        T::Currency::transfer(CurrencyId::xKSM, nominator, &Self::account_id(), amount)?;
    ...
    pub fn return_pending_balance_internal(
        ...
        T::Currency::transfer(CurrencyId::KSM, payer, nominator, pending_balance.balance)?;
