...
fn create_stream_works() {
    new_test_ext().execute_with(|| {
        ...
        // Dave cannot access
        assert_err!(
            Payroll::withdraw_from_stream(Origin::signed(DAVE), 0, 1),
...
fn cancel_stream_works_without_withdrawal() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Payroll::cancel_stream(Origin::signed(ALICE), 0));
        ...
        assert_err!(
            Payroll::withdraw_from_stream(Origin::signed(BOB), 0, 1),
...
fn withdraw_from_stream_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(Payroll::delta_of(stream.clone()), Ok(1));
        // Bob withdraws some
...
fn withdraw_from_with_slower_rate_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(Payroll::delta_of(stream.clone()), Ok(12));
        // Bob withdraws some
        assert_ok!(Payroll::withdraw_from_stream(
            Origin::signed(BOB),
            0,
        ...
        // Payroll is removed as balance goes zero
        assert_eq!(
            <Test as Config>::Assets::balance(DOT, &BOB) - before_bob,
            dollar(100)
        );
...
fn cancel_stream_works_with_withdrawal() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(Payroll::delta_of(stream.clone()), Ok(1));
        // Bob withdraws some
        assert_ok!(Payroll::withdraw_from_stream(Origin::signed(BOB), 0, 1));
        assert_eq!(Payroll::balance_of(stream.clone(), &2).unwrap(), dollar(1));
        // Time passes for 1 second
