use crate::{
    mock::MARKET_MOCK,
    mock::{Loans, Origin, Runtime, ALICE, DOT, NATIVE},
    tests::ExtBuilder,
...
macro_rules! rate_model_sanity_check {
    ($call:ident) => {
        ExtBuilder::default().build().execute_with(|| {
            // Invalid base_rate
            assert_noop!(
                Loans::$call(Origin::signed(ALICE), DOT, {
                    let mut market = MARKET_MOCK;
...
fn active_market_sets_state_to_active() {
    ExtBuilder::default().build().execute_with(|| {
        Loans::add_market(Origin::signed(ALICE), NATIVE, PENDING_MARKET_MOCK).unwrap();
        assert_eq!(Loans::market(&NATIVE).unwrap().state, MarketState::Pending);
        Loans::active_market(Origin::signed(ALICE), NATIVE).unwrap();
        assert_eq!(Loans::market(&NATIVE).unwrap().state, MarketState::Active);
    })
}
...
fn active_market_does_not_modify_unknown_market_currencies() {
    ExtBuilder::default().build().execute_with(|| {
        assert_noop!(
            Loans::active_market(Origin::signed(ALICE), NATIVE),
            Error::<Runtime>::MarketDoesNotExist
        );
    })
}

#[test]
fn add_market_does_not_insert_a_new_market_with_duplicated_currencies() {
    ExtBuilder::default().build().execute_with(|| {
        assert_noop!(
            Loans::add_market(Origin::signed(ALICE), DOT, MARKET_MOCK),
            Error::<Runtime>::MarketCurrencyAlreadyExists
        );
    })
}
...
fn add_market_ensures_that_market_state_must_be_pending() {
    ExtBuilder::default().build().execute_with(|| {
        Loans::add_market(Origin::signed(ALICE), NATIVE, MARKET_MOCK).unwrap();
        assert_eq!(Loans::market(&NATIVE).unwrap().state, MarketState::Pending);
    })
}
...
fn add_market_successfully_stores_a_new_market() {
    ExtBuilder::default().build().execute_with(|| {
        assert!(Loans::market(&NATIVE).is_err());
        Loans::add_market(Origin::signed(ALICE), NATIVE, MARKET_MOCK).unwrap();
        assert_eq!(Loans::market(&NATIVE).unwrap(), PENDING_MARKET_MOCK);
    })
}
...
fn update_market_does_not_modify_state() {
    ExtBuilder::default().build().execute_with(|| {
        Loans::update_market(Origin::signed(ALICE), DOT, PENDING_MARKET_MOCK).unwrap();
        assert_eq!(Loans::market(&DOT).unwrap().state, MarketState::Active);
    })
}

#[test]
fn update_market_ensures_that_is_not_possible_to_modify_unknown_market_currencies() {
    ExtBuilder::default().build().execute_with(|| {
        assert_noop!(
            Loans::update_market(Origin::signed(ALICE), NATIVE, MARKET_MOCK),
            Error::<Runtime>::MarketDoesNotExist
        );
    })
}
...
fn update_market_successfully_modifies_a_stored_market() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        Loans::update_market(Origin::signed(ALICE), DOT, {
            let mut market = MARKET_MOCK;
