...
//   UnixTime,
// }};
// use frame_system::{RawOrigin as SystemOrigin, Origin};
...
// use primitives::{AssetId, CurrencyId, Price, PriceWithDecimal, Rate, Ratio, TokenSymbol};
// use sp_runtime::traits::{One, StaticLookup, AtLeast32BitUnsigned};
// use sp_runtime::{ArithmeticError, FixedPointNumber, FixedU128, FixedPointOperand};
// use sp_std::prelude::*;
// use sp_std::vec;
...
// type AssetIdOf<T> =
// <<T as LoansConfig>::Assets as Inspect<<T as frame_system::Config>::AccountId>>::AssetId;
// type BalanceOf<T> =
// <<T as LoansConfig>::Assets as Inspect<<T as frame_system::Config>::AccountId>>::Balance;
...
//     Assets::force_create(SystemOrigin::Root.into(), DOT.into(), 1, true, 1).unwrap();
//     Assets::force_create(SystemOrigin::Root.into(), KSM.into(), 1, true, 1).unwrap();
//     Assets::mint_into(DOT, &account_id, INITIAL_AMOUNT.into()).unwrap();
//     Assets::mint_into(KSM, &account_id, INITIAL_AMOUNT.into()).unwrap();
...
//     <T as LoansConfig>::Assets::mint_into(DOT.into(), &caller, INITIAL_AMOUNT.into()).unwrap();
//     <T as LoansConfig>::Assets::mint_into(KSM.into(), &caller, INITIAL_AMOUNT.into()).unwrap();
// }
