...
impl From<OldAssetType> for Option<xcm::v2::MultiLocation> {
    fn from(val: OldAssetType) -> Self {
        match val {
            OldAssetType::Xcm(location) => Some(location),
        }
    }
}
...
enum PostUpgradeState<T: Config> {
    AssetIdType(Vec<(T::AssetId, T::AssetType)>),
    AssetTypeId(Vec<(T::AssetType, T::AssetId)>),
    AssetTypeUnitsPerSecond(Vec<(T::AssetType, u128)>),
    SupportedFeePaymentAssets(Vec<T::AssetType>),
}
    ...
    fn from(pre: PreUpgradeState<T>) -> PostUpgradeState<T> {
        match pre {
            PreUpgradeState::AssetIdType(items) => {
                let mut out: Vec<(T::AssetId, T::AssetType)> = Vec::new();
            ...
            PreUpgradeState::AssetTypeId(items) => {
                let mut out: Vec<(T::AssetType, T::AssetId)> = Vec::new();
                for (key, value) in items.into_iter() {
            ...
            PreUpgradeState::SupportedFeePaymentAssets(items) => {
                let mut out: Vec<T::AssetType> = Vec::new();
    ...
    T::AssetType: From<MultiLocation>,
{
    fn on_runtime_upgrade() -> Weight {
        ...
        AssetIdType::<T>::translate::<OldAssetType, _>(|_key, value| {
            db_weight_count.0 += 1;
            db_weight_count.1 += 1;
            let old_multilocation: Option<xcm::v2::MultiLocation> = value.into();
            let old_multilocation: xcm::v2::MultiLocation =
    ...
    fn pre_upgrade() -> Result<Vec<u8>, &'static str> {
        ...
        let asset_id_type_storage_data: Vec<_> =
            storage_key_iter::<T::AssetId, OldAssetType, Blake2_128Concat>(
                module_prefix,
        ...
        let asset_type_id_storage_data: Vec<_> =
            storage_key_iter::<OldAssetType, T::AssetId, Blake2_128Concat>(
                module_prefix,
                asset_type_id_storage_prefix,
            )
            .collect();
    ...
    fn post_upgrade(state: Vec<u8>) -> Result<(), &'static str> {
        ...
        let asset_id_type_storage_data: Vec<_> =
            storage_key_iter::<T::AssetId, T::AssetType, Blake2_128Concat>(
                module_prefix,
                asset_id_type_storage_prefix,
            )
            .collect();
        ...
        let asset_type_id_storage_data: Vec<_> =
            storage_key_iter::<T::AssetType, T::AssetId, Blake2_128Concat>(
                module_prefix,
                asset_type_id_storage_prefix,
            )
            .collect();
        ...
        let units_per_second_storage_data: Vec<_> =
            storage_key_iter::<T::AssetType, u128, Blake2_128Concat>(
                module_prefix,
                units_per_second_storage_prefix,
            )
            .collect();
