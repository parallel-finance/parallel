...
use frame_support::{
	debug, StorageMap,pallet_prelude::*,
};
use sp_runtime::{
...
pub mod module {
	...
	#[pallet::config]
	pub trait Config: frame_system::Config + CreateSignedTransaction<Call<Self>> + pallet_loans::Config {
	...
	impl<T:Config> Pallet<T> {
		...
		fn execute_liquidation(origin: OriginFor<T>, waiting_for_liquidation_vec: Vec<WaitingForLiquidation<T::AccountId>>)-> DispatchResultWithPostInfo {
			...
			for i in 0..waiting_for_liquidation_vec.len() {
				...
				let r = pallet_loans::Pallet::<T>::liquidate_borrow_internal(who.clone(), borrower.clone(), liquidate_token, repay_amount, collateral_token);
				match r {
					Ok(_) => debug::info!("success liquidate index: {:?}",i),
					Err(e) => {debug::error!("error invoke liquidate call: {:?}",e);continue;}
				}
			}
			// todo trigger event
	...
	impl<T: Config> Pallet<T> {
		fn liquidate(_block_number: T::BlockNumber) {
			...
			if let Ok(_guard) = lock.try_lock() {
				...
				'outer: for borrower in borrowers.iter() {
					...
					for currency_id in pallet_loans::Currencies::<T>::get().iter() {
						let currency_price = 
							match pallet_ocw_oracle::Prices::get(currency_id)
								.ok_or(Error::<T>::OracleCurrencyPriceNotReady){
									Ok((v,_)) => v,
									Err(e) => {debug::error!("error msg: {:?}",e);Price::MIN}
							};
						...
						let borrow_currency_amount =
							match pallet_loans::Pallet::<T>::borrow_balance_stored(borrower, currency_id){
								Ok(v) => v,
								Err(e) => {debug::error!("error get borrow balance: {:?}",e);continue 'outer;}
							};
						if borrow_currency_amount > 0 {
							if currency_price == Price::MIN {continue 'outer;}
							let borrow_currency_sum_price = 
							match borrow_currency_amount
								.checked_mul(currency_price)
								.ok_or(Error::<T>::CaculateError){
									Ok(v) => v,
									Err(e) => {debug::error!("error calculate borrow: {:?}",e);continue 'outer;}
								};
								
							classify_debts.push((currency_id.clone(),borrow_currency_amount,currency_price,borrow_currency_sum_price));
						}
						...
						if collateral_ctoken_amount > 0 {
							...
							let collateral_currency_amount = 
								match collateral_ctoken_amount.checked_mul(exchange_rate)
										...
										.ok_or(Error::<T>::CaculateError){
											Ok(v) => v,
											Err(e) => {debug::error!("error calculate collateral amount: {:?}",e);continue 'outer;}
								};
							...
							match collateral_currency_amount.checked_mul(currency_price).ok_or(Error::<T>::CaculateError) {
								Ok(v) => v,
								Err(e) => {debug::error!("error calculate collateral sum price: {:?}",e);continue 'outer;}
							};
							let liquidation_threshold = pallet_loans::LiquidationThreshold::<T>::get(currency_id);
							classify_collaterals.push((currency_id.clone(),collateral_currency_amount,currency_price,collateral_currency_sum_price,liquidation_threshold));
						}
					}

					if classify_debts.len() == 0 || classify_collaterals.len() == 0 {continue;}
					...
					debug::info!("_threshold_value: {:?}", collateral_liquidation_threshold_value);
					...
					for &(liquidate_token,debt_repay_amount,_,_debt_total_sum_price) in classify_debts.iter() {
						...
						for &(collateral_token,_,_,single_collateral_total_sum_pirce,_) in classify_collaterals.iter(){
							...
							let m: Price = 100;
							let repay_amount = 
								match single_collateral_total_sum_pirce
									.checked_mul(m)
									...
									.ok_or(Error::<T>::CaculateError)
									{
										Ok(v) => v,
										Err(e) => {
											debug::error!("error calculate waiting_for_liquidation_vec: {:?}",e);
											processing = false;
											Price::MIN
										}
									};
					...
					if waiting_for_liquidation_vec.len() == 0 || processing == false {continue;}
		...
		fn offchain_signed_tx(_borrower: T::AccountId, waiting_for_liquidation_vec: Vec<WaitingForLiquidation<T::AccountId>>) {
			// Get signer from ocw
